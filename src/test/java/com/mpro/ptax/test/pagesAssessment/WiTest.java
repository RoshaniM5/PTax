package com.mpro.ptax.test.pagesAssessment;

import java.io.IOException;
import org.testng.annotations.BeforeMethod;
import org.testng.annotations.Test;
import com.mpro.ptax.base.BasePage;
import com.mpro.ptax.components.GlobalButtons;
import com.mpro.ptax.enums.UserRole;
import com.mpro.ptax.model.WiData;
import com.mpro.ptax.pageobject.Assessment.Wi;
import com.mpro.ptax.test.auth.LoginTestFlow;
import com.mpro.ptax.test.base.BaseTest;
import com.mpro.ptax.test.dataprovider.WIDataProvider;

public class WiTest extends BaseTest{
	
	private LoginTestFlow login;
	private GlobalButtons globalbuttons;
	private BasePage basepage;
	private Wi wi;
	
	@BeforeMethod(alwaysRun =true)
	public void initFlow() {
		login 			= new LoginTestFlow();
		globalbuttons 	= new GlobalButtons();
		basepage 		= new BasePage();
		wi				= new Wi();
	}
	
	@Test(groups={"Sanity", "Wi", "Regression"}, alwaysRun =true, dataProvider = "WIDataProvider", dataProviderClass = WIDataProvider.class)
	public void verifyWiFlow(WiData data) throws IOException, InterruptedException {
		
		log.info("Testing WI Flow Test");
		login.loginAs(UserRole.Wi);
		
		log.info("ZoomedIn");
		basepage.zoomOut(75);
		    
		basepage.waitForLoaderToDisappear();
		
		log.info("Clicking cross icon on global buttons");
		globalbuttons.clickcrossIcon();
		 
		log.info("Inward Selected Successfully");
		basepage.selectLatestInward();
		
		log.info("ScrollDown is working correctly");
	    basepage.scrollUp();
	    
	    log.info("Screen1 details added successfully for New Assessment");
		wi.fileWIfileDetails(data);
		
		log.info("Screen2 details added successfully for New Flat Details");
		wi.fileWIfileDetailsNew();
		
//		String successmessage = wi.getSuccessMessage();
//		Assert.assertNotNull(successmessage, "SuccessMessageisNull");
//		Assert.assertTrue(successmessage.contains("Successfully updated details"), "wi Submission fails:" + successmessage);
		log.info("WI Flow validated successfully. Message: ");
		
	}
	

//		log.info("Logout completed successfully");
//		globalbuttons.Logout();
	}


